...
import { ContentQueryBuilder, createQueryBuilder } from "../../src/client/queryBuilder";
import type { TypedComponentApi, ConvexContext } from "../../src/client/wrapper";
import type { ContentEntry, PaginationResult } from "../../src/client/types";
...
describe("ContentQueryBuilder", () => {
	...
	describe("Full-Text Search", () => {
		it("search sets search query", () => {
			const builder = new ContentQueryBuilder(mockApi).search(
				"typescript tutorial",
			);
	...
	describe("all", () => {
		it("collects all results from multiple pages", async () => {
			...
			const mockCtx: ConvexContext = {
				...
				runQuery: vi
					.fn()
					.mockResolvedValueOnce(createMockPaginationResult(entries1, false))
					.mockResolvedValueOnce(createMockPaginationResult(entries2, false))
					.mockResolvedValueOnce(createMockPaginationResult(entries3, true)),
			};
