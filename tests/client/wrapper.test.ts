...
const createMockComponentApi = (): TypedComponentApi => ({
  ...
  contentTypeMutations: {
    createContentType: { _type: "mutation" } ,
    updateContentType: { _type: "mutation" } ,
    deleteContentType: { _type: "mutation" } ,
  },
  // Content entry queries
  contentEntries: {
    get: { _type: "query" } ,
    getBySlug: { _type: "query" } ,
    list: { _type: "query" } ,
    getVersionHistory: { _type: "query" } ,
    getVersion: { _type: "query" } ,
    compareVersions: { _type: "query" } ,
  },
  // Content entry mutations
  contentEntryMutations: {
    createEntry: { _type: "mutation" } ,
    updateEntry: { _type: "mutation" } ,
    deleteEntry: { _type: "mutation" } ,
    publishEntry: { _type: "mutation" } ,
    unpublishEntry: { _type: "mutation" } ,
    restoreEntry: { _type: "mutation" } ,
    duplicateEntry: { _type: "mutation" } ,
  },
  // Scheduled publish mutations
  scheduledPublish: {
    scheduleEntry: { _type: "mutation" } ,
  },
  // Version mutations
  versionMutations: {
    rollbackVersion: { _type: "mutation" } ,
  },
  // Media asset queries
  mediaAssets: {
    get: { _type: "query" } ,
  ...
  mediaAssetMutations: {
    createMediaAsset: { _type: "mutation" } ,
    updateMediaAsset: { _type: "mutation" } ,
    deleteMediaAsset: { _type: "mutation" } ,
    restoreMediaAsset: { _type: "mutation" } ,
    findMediaAssetReferences: { _type: "query" } ,
  },
  // Media upload mutations
  mediaUploadMutations: {
    generateUploadUrl: { _type: "mutation" } ,
  },
  // Media folder mutations
  mediaFolderMutations: {
    createMediaFolder: { _type: "mutation" } ,
    updateMediaFolder: { _type: "mutation" } ,
    deleteMediaFolder: { _type: "mutation" } ,
    getMediaFolder: { _type: "query" } ,
    listMediaFolders: { _type: "query" } ,
    moveMediaFolder: { _type: "mutation" } ,
    restoreMediaFolder: { _type: "mutation" } ,
    getMediaFolderByPath: { _type: "query" } ,
    getFolderTree: { _type: "query" } ,
  },
  // Media variants
  mediaVariants: {
    get: { _type: "query" } ,
    list: { _type: "query" } ,
    getBestVariant: { _type: "query" } ,
    getResponsiveSrcset: { _type: "query" } ,
    getAssetWithVariants: { _type: "query" } ,
    getPresets: { _type: "query" } ,
  },
  mediaVariantMutations: {
    createMediaVariant: { _type: "mutation" } ,
    requestVariantGeneration: { _type: "mutation" } ,
    generateFromPresets: { _type: "mutation" } ,
    deleteMediaVariant: { _type: "mutation" } ,
    deleteAssetVariants: { _type: "mutation" } ,
    restoreMediaVariant: { _type: "mutation" } ,
  },
  // Bulk operations
  bulkOperations: {
    bulkPublish: { _type: "mutation" } ,
    bulkUnpublish: { _type: "mutation" } ,
    bulkDelete: { _type: "mutation" } ,
    bulkUpdate: { _type: "mutation" } ,
    bulkRestore: { _type: "mutation" } ,
  },
...
describe("getUserRole Hook", () => {
  ...
  describe("hasUserRoleHook", () => {
    ...
    it("returns true when hook is configured", () => {
      const cms = createCmsClient(mockApi, {
        getUserRole: (_ctx, { userId: _userId }) => "admin",
      });
  ...
  describe("hasPermissionForUser", () => {
    ...
    it("returns allowed: false when user has no role", async () => {
      const cms = createCmsClient(mockApi, {
        getUserRole: (_ctx, { userId: _userId }) => null,
      });
    ...
    it("checks permissions for editor role", async () => {
      const cms = createCmsClient(mockApi, {
        getUserRole: (_ctx, { userId: _userId }) => "editor",
      });
    ...
    it("checks permissions for author role", async () => {
      const cms = createCmsClient(mockApi, {
        getUserRole: (_ctx, { userId: _userId }) => "author",
      });
    ...
    it("checks permissions for viewer role", async () => {
      const cms = createCmsClient(mockApi, {
        getUserRole: (_ctx, { userId: _userId }) => "viewer",
      });
    ...
    it("supports custom roles", async () => {
      ...
      const cms = createCmsClient(mockApi, {
        getUserRole: (_ctx, { userId: _userId }) => "moderator",
      });
...
import { MissingHookError, validateRequiredHooks } from "../../src/client/types";
...
describe("Required Hooks Validation", () => {
  ...
  describe("requireHooks configuration", () => {
    ...
    it("creates client when getUserRole hook is required and provided", () => {
      const cms = createCmsClient(testMockApi, {
        requireHooks: ["getUserRole"],
        getUserRole: async (_ctx, { userId: _userId }) => "admin",
      });
      expect(cms).toBeDefined();
    });
    ...
    it("creates client when all required hooks are provided", () => {
      const cms = createCmsClient(testMockApi, {
        ...
        getUserRole: async (_ctx, { userId: _userId }) => "editor",
        authorizationHooks: {
          beforeRbac: async () => ({ allowed: true }),
        },
        rateLimitHooks: {
...
describe("Authorization Enforcement", () => {
  ...
  describe("with getUserRole configured (proper authorization)", () => {
    ...
    it("throws UnauthorizedError when user has no role", async () => {
      const cms = createCmsClient(mockApi, {
        getUserRole: async (_ctx, { userId: _userId }) => null,
      });
