...
import {
  ...
  type IndexingStats,
} from "../../src/component/ragContentIndexer.js";
...
describe("RagContentIndexer - Entry Validation", () => {
  it("should only index published entries", () => {
    ...
    // Widen type to allow comparison - in production, status comes from DB as string
    const draftStatus: string = mockDraftEntry.status;
    const canIndexDraft = draftStatus === "published";
...
describe("RagContentIndexer - Event Payloads", () => {
  it("should extract content type from publish event payload", () => {
    const payload = mockPublishEvent.payload as { contentTypeName?: string; contentTypeId?: string };
...
describe("RagContentIndexer - Reindex Requests", () => {
  ...
  it("should validate entry is published before reindex", () => {
    // Widen types to allow runtime comparison testing
    const publishedStatus: string = "published";
