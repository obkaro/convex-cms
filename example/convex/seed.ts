...
export const seedAll = mutation({
  ...
  returns: v.null(),
  handler: async (ctx) => {
    const roadmapType = await cms.contentTypes.getByName(ctx, "roadmap_item");
    const changelogType = await cms.contentTypes.getByName(
      ctx,
      "changelog_entry"
    );

    if (!roadmapType || !changelogType) {
      console.error(
        "Content types not found. Run setup:syncContentTypes first."
      );
      return null;
    }

    const existingRoadmap = await cms.contentEntries.list(ctx, {
      contentTypeId: roadmapType._id,
    ...
    if (existingRoadmap.page.length > 0) {
      ...
      for (const item of roadmapItems) {
        ...
        await cms.contentEntries.publish(ctx, { id: entry._id });
      }
      console.log(`Created ${roadmapItems.length} roadmap items`);
    }

    const existingChangelog = await cms.contentEntries.list(ctx, {
      contentTypeId: changelogType._id,
      paginationOpts: { numItems: 1, cursor: null },
    });
    ...
    if (existingChangelog.page.length > 0) {
      ...
      for (const entry of changelogEntries) {
        const created = await cms.contentEntries.create(ctx, {
          contentTypeId: changelogType._id,
          data: entry,
        });
        await cms.contentEntries.publish(ctx, { id: created._id });
      }
