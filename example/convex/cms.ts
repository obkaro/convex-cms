...
export const cms = createCmsClient(components.convexCms, {
  ...
  /** All supported locales in the CMS */
  supportedLocales: ["en-US", "en-GB", "es-ES", "es-MX", "fr-FR", "de-DE", "ja-JP"],

  /** Custom fallback chains for locale resolution */
  localeFallbackChains: {
    // Spanish variants fall back to Spain Spanish, then English
    "es-MX": ["es-ES", "en-US"],
    "es-AR": ["es-ES", "en-US"],

    // British English falls back to US English
    "en-GB": ["en-US"],

    // Japanese falls back to English (no regional variants)
    "ja-JP": ["en-US"],

    // German has no specific fallbacks, uses default chain
    "de-DE": ["en-US"],
  ...
  features: {
    /** Enable version history for content entries */
    versioning: true,

    /** Enable multi-locale content management */
    localization: true,

    /** Enable scheduled publishing (future publish dates) */
    scheduling: true,

    /** Enable soft delete (content goes to trash before permanent deletion) */
    softDelete: true,
  },
  ...
  getUserRole: async (ctx, { userId }) => {
    // Look up the user in our users table by email (userId is typically email)
  ...
  authorizationHooks: {
    ...
    beforeRbac: async (context) => {
      // Example: Allow a service account to bypass all checks
      if (context.userId === "system-service-account") {
        return {
          allowed: true,
          skipRbac: true,
          reason: "System service account has full access",
        };
      }
