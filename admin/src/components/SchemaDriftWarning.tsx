...
export function SchemaDriftWarning({ api }: SchemaDriftWarningProps) {
	...
	if (!driftResults || driftResults.length === 0) {
		if (syncResult && (syncResult.created > 0 || syncResult.updated > 0 || syncResult.removedOrphans > 0)) {
			return (
				<Alert className="border-success/50 bg-success/10">
					...
					<AlertDescription className="text-diff-added-foreground/80">
						...
						{syncResult.removedOrphans > 0 && (
							<span>{syncResult.removedOrphans} orphaned type(s) removed.</span>
						)}
					</AlertDescription>
				</Alert>
			);
