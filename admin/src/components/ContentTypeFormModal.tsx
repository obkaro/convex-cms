 ...
 */
const FIELD_TYPE_INFO: Record<FieldType, { label: string; icon: string; description: string }> = {
  text: { label: 'Text', icon: 'Aa', description: 'Single line text input' },
  richText: { label: 'Rich Text', icon: '¶', description: 'Multi-line formatted text' },
  ...
  datetime: { label: 'Date & Time', icon: '📆', description: 'Date and time picker' },
  reference: { label: 'Reference', icon: '🔗', description: 'Link to another content entry' },
  ...
  const validationErrors = useMemo(() => {
    ...
    for (const field of fields) {
      if (!field.name.trim()) {
        ...
        errors.push(
          `Field "${field.name}" has an invalid name format`
        );
      }
      if (!field.label.trim()) {
      ...
      if (
        ...
        errors.push(`${field.type} field "${field.label}" requires at least one option`);
      }
    }
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body modal-body-scrollable">
            ...
            <div className="form-section">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  disabled={isSubmitting}
                  className={!isValidMachineName(machineName) && machineName ? 'input-error' : ''}
                ...
                {fields.map((field, index) => (
                  <div
                    ...
                    className={`field-list-item ${
                      activeFieldIndex === index ? 'field-list-item--active' : ''
                    }`}
                    onClick={() => {
                  ...
                  >
                    ...
                    <div className="field-list-item-info">
                      <span className="field-list-item-label">{field.label}</span>
                      ...
                      onChange={(e) =>
                        updateField(activeFieldIndex, {
                          ...
                          name: machineNameManuallyEdited
                            ? activeField.name
                            : generateMachineName(e.target.value) || activeField.name,
                        })
                      }
                      disabled={isSubmitting}
            ...
            {/* Error display */}
            {submitError && (
              <div className="form-error" role="alert" data-testid="submit-error">
                {submitError}
              </div>
            )}
          </div>
