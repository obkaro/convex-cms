 ...
 */
export function extractTextFromJson(value: unknown, maxDepth: number = 5): string {
  ...
  if (typeof value === "object") {
    ...
    const priorityKeys = ["text", "content", "value", "label", "title", "name", "description"];
 ...
 *   args: { entryId: v.id("content_entries") },
...
function detectSemanticType(text: string): ChunkSemanticType {
  ...
  // Check for headings
  if (trimmed.match(/^#{1,6}\s/) || (trimmed.length < 100 && !trimmed.includes("\n"))) {
