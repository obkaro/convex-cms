...
export async function logAuditEntry(
	...
	if (
		...
		summary = `Updated ${changedFields.length} field${
	...
	switch (action) {
		...
		case "updated":
			if (changedFields && changedFields.length > 0) {
				return `Updated ${changedFields.length} field${
					changedFields.length > 1 ? "s" : ""
		...
		if (changedFields && changedFields.length > 0) {
			const fieldDiffs = changedFields.map((field) => ({
				...
				previousValue: previousState
					? (previousState as Record<string, unknown>)[field]
					: undefined,
				newValue: newState
					? (newState as Record<string, unknown>)[field]
					: undefined,
			}));
