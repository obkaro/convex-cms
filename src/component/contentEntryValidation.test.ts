...
describe("Content Entry Validation", () => {
  ...
  describe("Reference field validation", () => {
    ...
    it("should validate max items constraint", () => {
      ...
      const errors = validateReferenceField(["tag_1", "tag_2", "tag_3"], fieldDef);
      expect(errors).toHaveLength(1);
  ...
  describe("Complex schema validation", () => {
    const blogPostSchema: ContentTypeSchema = {
      ...
      displayName: "Blog Post",
      fields: [
        { name: "title", label: "Title", type: "text", required: true, options: { minLength: 5, maxLength: 100 } },
        { name: "slug", label: "Slug", type: "text", required: true, options: { pattern: "^[a-z0-9-]+$" } },
        { name: "content", label: "Content", type: "richText", required: true },
        { name: "views", label: "Views", type: "number", required: false, options: { min: 0 } },
    ...
    it("should report multiple errors for invalid blog post", () => {
      ...
      expect(result.errors.some((e) => e.field === "slug" && e.code === "PATTERN_MISMATCH")).toBe(true);
      expect(result.errors.some((e) => e.field === "content" && e.code === "REQUIRED")).toBe(true);
      expect(result.errors.some((e) => e.field === "views" && e.code === "MIN_VALUE")).toBe(true);
    });
  });
...
describe("Content Entry Validation Logic Patterns", () => {
  ...
  describe("Validation options patterns", () => {
    ...
    it("should allow disabling reference validation", () => {
      const options: { validateReferences?: boolean } = { validateReferences: false };
      const validateReferences = options?.validateReferences ?? true;
