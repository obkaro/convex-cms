...
import { BULK_OPERATION_BATCH_SIZE } from "./validators.js";
...
export const migrateContentTypeArgs = v.object({
  ...
  entryIds: v.optional(v.array(v.id("content_entries"))),
...
export const migrateContentType = mutation({
  ...
  handler: async (ctx, args): Promise<MigrationResult> => {
    ...
    for (const op of migrations) {
      if (op.type === "RENAME_FIELD" && (!op.oldFieldName || !op.newFieldName)) {
        throw new Error(
    ...
    if (statusFilter && statusFilter.length > 0) {
      entries = entries.filter((e) =>
        statusFilter.includes(e.status as "draft" | "published" | "archived" | "scheduled")
      );
    }
        ...
        if (dryRun) {
          ...
          if (createVersionSnapshots) {
            await ctx.db.insert("content_versions", {
              ...
              publishedAt:
                entry.status === "published" ? entry.lastPublishedAt : undefined,
            });
