 ...
 *
 * Architecture:
 * 1. Events are captured via the event emitter system (cms_events table)
...
export const createWebhook = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const webhookId = await ctx.db.insert("webhook_configs", {
      name,
      description,
      url,
      secret,
  ...
  handler: async (ctx, args) => {
    ...
    return webhooks.filter((webhook) => {
      ...
      if (
        ...
        !webhook.resourceTypes.includes(resourceType as any)
      ) {
        return false;
      }
...
export const createDelivery = internalMutation({
  ...
  returns: v.id("webhook_deliveries"),
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (errorMessage.includes("aborted")) {
        await handleDeliveryFailure(
          ...
          deliveryData.attemptCount + 1,
          deliveryData.maxAttempts,
          `Request timeout after ${webhook.timeoutMs || DEFAULT_WEBHOOK_CONFIG.defaultTimeoutMs}ms`,
          undefined,
          undefined,
          durationMs
        );
 ...
 *
 * This mutation:
 * 1. Gets unprocessed events from cms_events
...
export const testWebhook = mutation({
  ...
  returns: v.object({
    success: v.boolean(),
    message: v.string(),
    deliveryId: v.optional(v.id("webhook_deliveries")),
  }),
  handler: async (ctx, args) => {
    ...
    // Create a test event
    const testEventId = await ctx.db.insert("cms_events", {
