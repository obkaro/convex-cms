  ...
  describe("deleteContentType", () => {
    ...
    it("cascade soft deletes entries when cascade is true", async () => {
      ...
      const entryIds = await t.run(async (ctx) => {
        const id1 = await ctx.db.insert("content_entries", {
          contentTypeId: contentType._id,
          slug: "post-1",
          status: "draft",
          data: { title: "Post 1" },
        ...
        const id2 = await ctx.db.insert("content_entries", {
          contentTypeId: contentType._id,
          slug: "post-2",
          status: "published",
          data: { title: "Post 2" },
    ...
    it("cascade hard deletes entries and versions when cascade and hardDelete are true", async () => {
      ...
      const { entryId, versionIds } = await t.run(async (ctx) => {
        const eId = await ctx.db.insert("content_entries", {
          contentTypeId: contentType._id,
          slug: "post-1",
          status: "published",
          data: { title: "Post 1" },
        ...
        const vId1 = await ctx.db.insert("content_versions", {
          entryId: eId,
          versionNumber: 1,
        ...
        const vId2 = await ctx.db.insert("content_versions", {
          entryId: eId,
          versionNumber: 2,
          data: { title: "Post 1" },
          slug: "post-1",
