...
import { contentQueryArgs, contentEntryDoc } from "./validators.js";
...
describe("Content Entries List Query", () => {
  ...
  describe("Filter logic patterns", () => {
    ...
    it("should filter by multiple statuses when statusIn is specified", () => {
      ...
      expect(filtered.every((e) => ["draft", "scheduled"].includes(e.status))).toBe(true);
    });
    ...
    it("should return empty array for non-existent content type name", () => {
      const contentType = null; // Simulates content type not found
      const shouldReturnEmpty =
        !contentType ||
        !(contentType as any).isActive ||
        (contentType as any).deletedAt !== undefined;
  ...
  describe("Status resolution logic (statusIn vs status)", () => {
    ...
    it("should convert single status to array when statusIn is not provided", () => {
      ...
      const statusIn: string[] | undefined = undefined;

      const resolvedStatuses = statusIn && statusIn.length > 0
        ? statusIn
        : status
          ? [status]
          : undefined;
