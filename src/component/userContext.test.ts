...
describe("resolveUserRole", () => {
  ...
  it("should resolve role using getUserRole hook", async () => {
    const role = await resolveUserRole("user_admin", mockGetUserRole);
    expect(role).toBe("admin");
    expect(mockGetUserRole).toHaveBeenCalledWith({ userId: "user_admin" });
  });

  it("should return null when hook is not provided", async () => {
    const role = await resolveUserRole("user_123");
    expect(role).toBeNull();
  });

  it("should return null for unknown users", async () => {
    const role = await resolveUserRole("unknown_user", mockGetUserRole);
    expect(role).toBeNull();
  });
  ...
  it("should throw UserContextError when hook fails", async () => {
    ...
    await expect(resolveUserRole("user_123", failingHook)).rejects.toThrow(UserContextError);
    await expect(resolveUserRole("user_123", failingHook)).rejects.toMatchObject({
      code: "HOOK_ERROR",
    });
  });
});
