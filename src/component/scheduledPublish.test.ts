...
describe("Scheduled Publish", () => {
  ...
  async function createContentType(t: ReturnType<typeof convexTest>) {
    return await t.run(async (ctx) => {
      return await ctx.db.insert("content_types", {
        name: "blog_post",
  ...
  async function createDraftEntry(
    ...
    data: Record<string, unknown> = { title: "Test Post", content: "Test content" }
  ) {
    return await t.run(async (ctx) => {
  ...
  describe("cancelScheduledPublish", () => {
    test("cancels a scheduled entry and reverts to draft", async () => {
      ...
      const result = await t.mutation(api.scheduledPublish.cancelScheduledPublish, {
        id: entryId,
      });
  ...
  describe("getScheduledEntries", () => {
    ...
    test("filters by content type", async () => {
      ...
      await t.run(async (ctx) => {
        await ctx.db.insert("content_entries", {
          contentTypeId: blogTypeId as any,
          slug: "blog-post",
          status: "scheduled",
        ...
        await ctx.db.insert("content_entries", {
          contentTypeId: pageTypeId as any,
          slug: "page",
          status: "scheduled",
          data: { title: "Page" },
          version: 1,
