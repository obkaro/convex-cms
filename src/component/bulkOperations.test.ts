...
describe("Bulk Publish Integration Tests", () => {
  it("should publish multiple draft entries", async () => {
    ...
    const published1 = await t.query(api.contentEntries.get, { id: entry1._id });
    const published2 = await t.query(api.contentEntries.get, { id: entry2._id });
    const published3 = await t.query(api.contentEntries.get, { id: entry3._id });
  ...
  it("should handle partial success (some entries succeed, some fail)", async () => {
    ...
    const deletedEntry = await t.mutation(api.contentEntryMutations.createEntry, {
      contentTypeId: contentType._id,
      data: { title: "Deleted" },
    });
    await t.mutation(api.contentEntryMutations.deleteEntry, {
...
describe("Bulk Unpublish Integration Tests", () => {
  it("should unpublish multiple published entries", async () => {
    ...
    await t.mutation(api.contentEntryMutations.publishEntry, { id: entry1._id });

    const entry2 = await t.mutation(api.contentEntryMutations.createEntry, {
      contentTypeId: contentType._id,
      data: { title: "Entry 2" },
    });
    await t.mutation(api.contentEntryMutations.publishEntry, { id: entry2._id });
    ...
    const unpublished1 = await t.query(api.contentEntries.get, { id: entry1._id });
    const unpublished2 = await t.query(api.contentEntries.get, { id: entry2._id });
  ...
  it("should preserve publication timestamps after unpublish", async () => {
    ...
    const unpublished = await t.query(api.contentEntries.get, { id: entry._id });
    ...
    const contentType = await t.mutation(
      api.contentTypeMutations.createContentType,
      {
        ...
        fields: [
          ...
          { name: "description", label: "Description", type: "text", required: false, searchable: true },
        ],
      }
    );
...
describe("Bulk Operations Edge Cases", () => {
  it("should handle mixed success/failure across entries", async () => {
    ...
    await t.mutation(api.contentEntryMutations.deleteEntry, { id: deletedEntry._id });

    const archivedEntry = await t.mutation(api.contentEntryMutations.createEntry, {
      contentTypeId: contentType._id,
      data: { title: "Archived" },
      status: "archived",
    });
