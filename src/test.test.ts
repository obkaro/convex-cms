...
describe("contentEntryFactory", () => {
  ...
  const fakeContentTypeId = "content_type_123";
...
describe("mediaAssetFactory", () => {
  ...
  const fakeStorageId = "storage_123";
...
describe("mediaFolderFactory", () => {
  ...
  describe("child", () => {
    it("creates a child folder with correct path", () => {
      ...
      expect(folder.path).toBe("/images/2024");
...
describe("Assertions", () => {
  ...
  describe("assertContentEntry", () => {
    it("passes for valid entry", () => {
      const entry = {
        _id: "id123",
        _creationTime: Date.now(),
        ...contentEntryFactory.minimal("type_123"),
      };
  ...
  describe("assertMediaAsset", () => {
    it("passes for valid asset", () => {
      const asset = {
        _id: "id123",
        _creationTime: Date.now(),
        ...mediaAssetFactory.image("storage_123"),
      };
  ...
  describe("assertStatus", () => {
    it("passes for matching status", () => {
      expect(() =>
        assertStatus({ status: "draft" }, "draft")
      ).not.toThrow();
    });

    it("throws for mismatched status", () => {
      expect(() =>
        assertStatus({ status: "draft" }, "published")
      ).toThrow();
    });
  });
  ...
  describe("assertDeleted", () => {
    it("passes for deleted document", () => {
      expect(() =>
        assertDeleted({ deletedAt: Date.now() })
      ).not.toThrow();
    });
  ...
  describe("assertNotDeleted", () => {
    ...
    it("throws for deleted document", () => {
      expect(() =>
        assertNotDeleted({ deletedAt: Date.now() })
      ).toThrow();
    });
  });
  ...
  describe("assertField", () => {
    ...
    it("throws when field doesn't match expectations", () => {
      ...
      expect(() =>
        assertField(fields, "title", { required: false })
      ).toThrow();
    });
  });
});
