...
import { resolveConfig, type ComponentConfig } from "./types.js";
...
describe("Custom Roles Integration", () => {
      ...
      expect(
        allRoles["blog-author"].permissions.some(
          (p) =>
            ...
            p.action === "create" &&
            p.contentTypes?.includes("blog_post")
        )
      ).toBe(true);
    });
  ...
  describe("Multi-tenant CMS Setup", () => {
    ...
    const customRoles = [legalTeamEditor, marketingEditor];
  ...
  describe("Config Integration", () => {
    const testRole = createCustomRole({
      name: "test-role",
