...
import type { TypedComponentApi, EnhancedCmsClient } from "../wrapper.js";
...
export class TypedContentEntriesApiImpl<
  TSchema extends ContentSchemaInstance<Record<string, ContentTypeDefinition>>
> {
  private baseClient: EnhancedCmsClient;
  private schema: TSchema;

  constructor(baseClient: EnhancedCmsClient, schema: TSchema) {
  ...
  async get<TName extends ValidContentTypeName<TSchema>>(
    ...
    return entry as unknown as TypedContentEntry<SchemaDataType<TSchema, TName>>;
  }
  ...
  async list<TName extends ValidContentTypeName<TSchema>>(
    ...
    return {
      page: result.page as unknown as TypedContentEntry<SchemaDataType<TSchema, TName>>[],
 ...
 * The returned client extends the base `EnhancedCmsClient` with additional
...
>(
  ...
  const { createCmsClient } = require("../index.js") as {
    createCmsClient: (api: TypedComponentApi, config?: ComponentConfig) => EnhancedCmsClient;
  };
